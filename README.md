...
Bond Bot first reads `GEMINI_API_KEY` from the web-server environment and uses the stable `gemini-3.5-flash` model through Google's server-side Generate Content API. For local XAMPP development, copy `config/openai.local.php.example` to `config/openai.local.php`, insert the Gemini key under `gemini_api_key`, and never commit that local file. `GEMINI_MODEL` and `GEMINI_BASE_URL` are optional overrides. If no Gemini key exists, a direct `OPENAI_API_KEY` uses the OpenAI Responses API. When neither direct key exists on Vercel, Bond Bot falls back to the deployment's automatic `VERCEL_OIDC_TOKEN` with Vercel AI Gateway; that route currently requires a valid payment card. `AI_GATEWAY_API_KEY`, `AI_GATEWAY_MODEL`, and `AI_GATEWAY_BASE_URL` remain optional overrides.

## Vercel deployment

`vercel.json` uses the community `vercel-php` runtime for the PHP APIs and thin Vercel entry bridges in `api/`; static game assets are served directly. Direct web access to configuration, database and private Admin bootstrap source paths is blocked. Configure `SEVENTH_REACTION_DB_HOST`, `SEVENTH_REACTION_DB_PORT`, `SEVENTH_REACTION_DB_NAME`, `SEVENTH_REACTION_DB_USER`, `SEVENTH_REACTION_DB_PASS` and `SEVENTH_REACTION_DB_TLS=true` with a network-accessible MySQL-compatible database in Vercel before enabling registration, login, cloud saves, leaderboards or the Admin Website. TiDB's standard `TIDB_HOST`, `TIDB_PORT`, `TIDB_DATABASE`, `TIDB_USER` and `TIDB_PASSWORD` variables are also accepted. Without those variables, the public game remains playable through **PLAY OFFLINE DEMO**, but database features intentionally report that the server is unavailable. Add `GEMINI_API_KEY` as an encrypted Production environment variable to activate the preferred free-tier Bond Bot provider; a direct `OPENAI_API_KEY` remains supported as a paid fallback.

Without a Gemini key, a direct OpenAI key or a usable Vercel AI Gateway identity, the game automatically uses its verified EC015 offline explanations and clearly labels that response as a fallback.
